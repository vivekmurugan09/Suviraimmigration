...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initGlobalContactModal);
} else {
    initGlobalContactModal();
}

// Inject and handle Global "Send Us a Message" Contact Modal
function initGlobalContactModal() {
    if (document.getElementById('globalContactModal')) return;

    const modalHTML = `
    <div class="contact-modal-overlay" id="globalContactModal">
        <div class="contact-modal-card">
            <div class="contact-modal-header">
                <h3><i class="fas fa-envelope-open-text" style="color:#FF9F1C;"></i> Send Us a Message</h3>
                <button class="contact-modal-close" onclick="closeContactModal()"><i class="fas fa-times"></i></button>
            </div>
            <div class="contact-modal-body">
                <p style="color: #64748B; font-size: 0.9rem; margin-bottom: 18px;">
                    Fill out your details below and our senior specialists will get back to you within 24 hours.
                </p>
                <form id="globalContactModalForm" onsubmit="handleGlobalModalSubmit(event)">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px;">
                        <div>
                            <label style="display: block; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #64748B; margin-bottom: 5px;">Full Name *</label>
                            <input type="text" id="gmName" required placeholder="Enter full name" style="width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;font-size:0.9rem;outline:none;box-sizing:border-box;">
                        </div>
                        <div>
                            <label style="display: block; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #64748B; margin-bottom: 5px;">Phone / WhatsApp *</label>
                            <input type="tel" id="gmPhone" required placeholder="+91 99403 52086" style="width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;font-size:0.9rem;outline:none;box-sizing:border-box;">
                        </div>
                    </div>

                    <div style="margin-bottom: 14px;">
                        <label style="display: block; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #64748B; margin-bottom: 5px;">Email Address *</label>
                        <input type="email" id="gmEmail" required placeholder="your.email@example.com" style="width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;font-size:0.9rem;outline:none;box-sizing:border-box;">
                    </div>

                    <div style="margin-bottom: 14px;">
                        <label style="display: block; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #64748B; margin-bottom: 5px;">Service Required</label>
                        <select id="gmService" style="width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;font-size:0.9rem;outline:none;box-sizing:border-box;">
                            <option value="Canada PR / Express Entry">🇨🇦 Canada (PR / Express Entry)</option>
                            <option value="Australia PR Points">🇦🇺 Australia (Subclass 189 / 190)</option>
                            <option value="Germany Opportunity Card 2026">🇩🇪 Germany (Opportunity Card)</option>
                            <option value="Overseas Study Visa">🎓 Overseas Study Visa</option>
                            <option value="Visitor / Tourist Visa">✈️ Visitor / Tourist Visa</option>
                            <option value="SOP & Resume Writing">📄 SOP & Resume Service</option>
                            <option value="PCC & Apostille Attestation">stamp PCC & Apostille</option>
                            <option value="General Consultation">General Consultation</option>
                        </select>
                    </div>

                    <div style="margin-bottom: 18px;">
                        <label style="display: block; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #64748B; margin-bottom: 5px;">Your Message *</label>
                        <textarea id="gmMsg" rows="3" required placeholder="Describe your query or requirements..." style="width:100%;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;font-size:0.9rem;outline:none;resize:none;box-sizing:border-box;"></textarea>
                    </div>

                    <button type="submit" style="width:100%;background:linear-gradient(135deg, #F87034, #E8501A);color:white;border:none;padding:13px;border-radius:12px;font-weight:700;font-size:1rem;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 8px 24px rgba(248,112,52,0.35);">
                        <i class="fas fa-paper-plane"></i> Send Direct Inquiry to Suvira Team
                    </button>
                </form>

                <div id="gmSuccess" style="display:none;background:#F0FDF4;border:1.5px solid #86EFAC;border-radius:12px;padding:16px;margin-top:16px;color:#15803D;font-weight:600;font-size:0.9rem;align-items:center;gap:10px;">
                    <i class="fas fa-check-circle" style="font-size:1.3rem;"></i>
                    <span>Thank you! Your message has been sent to reachus@suviraimmigration.com. Our Chennai team will contact you within 24 hours.</span>
                </div>
            </div>
        </div>
    </div>`;
    ...
    // Bind click events on all contact-float elements
    document.querySelectorAll('.contact-float').forEach(btn => {
...
function handleGlobalModalSubmit(e) {
    e.preventDefault();
    ...
    const service = document.getElementById('gmService').value;
    ...
    try {
        ...
        formData.append("_subject", `New Popup Inquiry: ${name} (${service})`);
    ...
    const box = document.getElementById('gmSuccess');
    if (box) {
        box.style.display = 'flex';
        setTimeout(() => {
            document.getElementById('globalContactModalForm').reset();
